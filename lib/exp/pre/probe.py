...
class Probe(Preloader):
    ...
    def diff_next(self, end=-1, qs=2):
        ...
        ratio = 1.0/qs
        ret = []
        for imds in self.video.frame_set(start=0, end=end, qsize=qs+1):
            with ExpTimer(verbose=False) as t:
                imgs = [im["img"] for im in imds]
                diff = self.__get_local_avgimg(imgs, ratio)
            data = dict(htime=t.msecs, ms=imds[-1]['ms'], diff=diff,
                        frame_id=imds[-1]['fn'])
            ret.append(data)
            yield imds[-1]['fn'], t.tstr()
