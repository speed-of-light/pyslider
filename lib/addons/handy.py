...
class HandyPlot:
    ...
    """
    @staticmethod
    def table(fig, img_list, titles=None, row=2, col=2):
        ...
        for i, img in enumerate(img_list, 1):
            ...
            title = "img_{}".format(i) if titles is None else titles[i-1]
            ax.set_title(title)
            ax.imshow(img)
            if i >= b:
                break
...
class HandyStat:
    ...
    def posible_kf(afr=None, thres=0):
        ...
        By selecting the difference of slide with max key points count
        and mean key points count sort and reorder
        Params:
        ...
        Usage: run this twice...
          ...
          limit = 1-len(slides)*(torance)/1.0*len(first)
          second = posible_kf(first.quantile(limit).val)
        """
        gc = []
        gv = []
        gi = []
...
class ProgressBar:
    ...
    def anistr(self, new_amount):
        ...
        all_full = self.width - 2
        num_hashes = int(round((percent_done / 100.0) * all_full))
        pstr = '[' + self.fill_char * num_hashes + \
               ' ' * (all_full - num_hashes) + ']'
