...
class Matcher(ExpCommon):
    ...
    def plot_ransac(self, fid, sid, res):
        ra = Ransac()
    ...
    def plot_img_match(self, qimg, qkp, timg, tkp, match):
      h1, w1 = qimg.shape[:2]
      h2, w2 = timg.shape[:2]
    ...
    def plot_id_match(self, fin, sin, fkp, skp, mresult):
        ...
        if isinstance(fin, int):
            ...
            fimg = fin
        vimg = self.plot_img_match(fimg, fkp, simg, skp, mresult)
        return vimg
